...
def _get_sandbox_dir() -> str:
    """Get sandbox directory, supporting test monkeypatching."""
    from koro.core import config

    return config.SANDBOX_DIR


def _get_working_dir() -> str:
    """Get working directory, supporting test monkeypatching."""
    from koro.core import config

    return config.CLAUDE_WORKING_DIR
...
def load_megg_context(working_dir: str = None) -> str:
    ...
    """
    cwd = working_dir or _get_working_dir()
    try:
...
class ClaudeClient:
    ...
    def __init__(
        ...
        """
        self.sandbox_dir = sandbox_dir or _get_sandbox_dir()
