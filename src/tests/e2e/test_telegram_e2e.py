...
class TelegramTester:
    ...
    async def verify_connectivity(self) -> tuple[bool, str]:
        ...
        try:
            ...
            try:
                await self.bot.delete_message(chat_id=self.chat_id, message_id=test_msg.message_id)
            except TelegramError:
                pass

            return True, f"Connectivity OK - tester bot (@{me.username}) can send to chat {self.chat_id}"
    ...
    async def wait_for_response(
        self, timeout: int = 30, from_bot_username: str = None
    ) -> str | None:
        ...
        Args:
            ...
            from_bot_username: Optional username to filter messages from
            ...
            try:
                ...
                for update in updates:
                    ...
                    if update.message and update.message.chat_id == self.chat_id:
                        # Check if this is from a bot (not from our tester bot)
                        if update.message.from_user and update.message.from_user.is_bot:
                            # Skip messages from our own tester bot
                            if update.message.from_user.id == (await self.bot.get_me()).id:
                                continue

                            # Check if from specific bot username if requested
                            if from_bot_username:
                                if update.message.from_user.username == from_bot_username:
                                    return update.message.text or update.message.caption
                            else:
                                # Any other bot's message
                                return update.message.text or update.message.caption
